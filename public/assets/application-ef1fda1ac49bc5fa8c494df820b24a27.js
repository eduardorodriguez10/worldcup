...
function updateOptionsR16(selected, disabled, next_round){
	...
	function hideTeamsW(w_number){
		for(var t = 1; t<33; t++){
				if(!($("#bracket_w"+w_number.toString()+" option[value="+t.toString()+"]").selected)){
		...
		for (var i = 97; i <105; i++){
			var e = document.getElementById("bracket_"+String.fromCharCode(i)+"1");
				...
				if(value){
					...
					console.log("bracket_"+String.fromCharCode(i)+"2 value"+value);
					$("#bracket_"+String.fromCharCode(i)+"1").removeClass("notselected");
					$("#bracket_"+String.fromCharCode(i)+"1").addClass("selected");
				}
				else
					console.log("#bracket_"+String.fromCharCode(i)+"1 has no value ");
			}
